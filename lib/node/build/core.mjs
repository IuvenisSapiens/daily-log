...
function prefixRootRelativeHtmlPaths(html, websiteUrl) {
  if (!websiteUrl) {
    return html;
  }
  ...
  const replaceAttr = (source) => source.replace(/\b(href|src|action)=(['"])\/(?!\/)([^"']*)\2/g, (_match, attr, quote, path) => {
    return `${attr}=${quote}${normalizedBase}/${path}${quote}`;
  });

  const replaced = replaceAttr(html)
    .replace(/url\((['"]?)\/(?!\/)([^\)'"]+)\1\)/g, (_match, quote, path) => {
      return `url(${quote}${normalizedBase}/${path}${quote})`;
    })
    .replace(/url\(&quot;\/(?!\/)([^&]*)&quot;\)/g, (_match, path) => {
      return `url(&quot;${normalizedBase}/${path}&quot;)`;
    })
    .replace(/url\(&#39;\/(?!\/)([^&]*)&\#39;\)/g, (_match, path) => {
      return `url(&#39;${normalizedBase}/${path}&#39;)`;
    });
...
async function compileAndStageTypstTasks(tasks, stagingSiteDir, jobs, statusMap, websiteUrl = null) {
  ...
  const results = await runPool(
    ...
    async (task) => {
      ...
      if (websiteUrl && task.outputRel.toLowerCase().endsWith(".html")) {
        const html = readFileSync(task.stagingOutputPath, "utf8");
        const rewritten = prefixRootRelativeHtmlPaths(html, websiteUrl);
        if (rewritten !== html) {
          writeFileSync(task.stagingOutputPath, rewritten, "utf8");
        }
      }
...
export async function buildSite({ outDirName, jobs, configPath, cacheRoot }) {
  ...
  await compileAndStageTypstTasks(allCompileTasks, paths.stagingSiteDir, jobs, statusMap, siteInputs.websiteUrl);
